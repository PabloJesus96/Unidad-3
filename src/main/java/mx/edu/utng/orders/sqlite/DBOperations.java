...
/**
 * Created by Husky Siberiano on 08/02/2017.
 */
...
public final class DBOperations {
    ...
    public Cursor getDepartments(){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s",
                OrderDB.Tables.DEPARTMENTS);
        return  database.rawQuery(sql, null);
    }

    public Cursor getDepartmentsById(String id){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s" +
                        " WHERE %s=?",
                OrderDB.Tables.DEPARTMENTS,
                OrderContract.Departments.DEPTO_NO);
        String[] whereArgs = {id};
        return database.rawQuery(sql, whereArgs);
    }

    public String insertDepartments(Departments department){
        SQLiteDatabase database = db.getWritableDatabase();
        String idDepartment = OrderContract.Departments.generatedIdDepartment();
        ContentValues values = new ContentValues();
        values.put(OrderContract.Departments.DEPTO_NO, idDepartment);
        values.put(OrderContract.Departments.DEPTO_NAME, department.getDeptoName());
        values.put(OrderContract.Departments.DEPTO_PHONE, department.getDeptoPhone());
        database.insertOrThrow(OrderDB.Tables.DEPARTMENTS, null, values);
        return idDepartment;
    }
    ...
    public boolean updateDepartment(Departments department){
        ...
        values.put(OrderContract.Departments.DEPTO_NAME, department.getDeptoName());
        values.put(OrderContract.Departments.DEPTO_PHONE, department.getDeptoPhone());
        String whereClause = String.format("%s=?", OrderContract.Departments.DEPTO_NO);
        String[] whereArgs = {department.getDeptoNo()};
        int result = database.update(OrderDB.Tables.DEPARTMENTS,
                values, whereClause, whereArgs);
        return result>0;
    }
    ...
    public boolean deleteDepartment(String idDepartment){
        SQLiteDatabase database = db.getWritableDatabase();
        String whereClause = OrderContract.Departments.DEPTO_NO + "=?";
        String[] whereArgs = {idDepartment};
        int result =  database.delete(OrderDB.Tables.DEPARTMENTS,
                whereClause, whereArgs);
        return result > 0;
    }
